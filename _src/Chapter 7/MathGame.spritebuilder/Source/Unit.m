...
-(id)initCommon
{
	if ((self=[super initWithImageNamed:@"imgUnit.png"]))
	{
		...
		self.lblValue = [CCLabelBMFont labelWithString:@"1" fntFile:@"bmScoreFont.fnt"];
		//self.lblValue.scale = 1.5;
		self.lblValue.position = ccp(self.contentSize.width/2, self.contentSize.height * 0.55f);
...
-(BOOL)moveUnitDidIncreaseNumber
{
	...
	if (self.isFriendly)
	{
		if ((gridX == 1 && self.direction == DirLeft) ||
			(gridX == 9 && self.direction == DirRight) ||
			(gridY == 1 && self.direction == DirUp) ||
			(gridY == 9 && self.direction == DirDown)) //if didn't move, it's standing
		{
...
-(void)touchMoved:(CCTouch *)touch withEvent:(CCTouchEvent *)event
{
	...
	if (!self.isBeingDragged && ccpDistance(touchPos, self.touchDownPos) > 20)
	{
		...
		if ((((MainScene*)self.parent).tutorialPhase == 1 && self.dragDirection != DirRight) ||
	...
	if (self.isBeingDragged)
	{
		...
		if (self.direction == DirAtWall)
		{
			if ((self.gridPos.y == 1 && self.dragDirection == DirUp) ||
				(self.gridPos.y == 9 && self.dragDirection == DirDown) ||
				(self.gridPos.x == 1 && self.dragDirection == DirLeft) ||
				(self.gridPos.x == 9 && self.dragDirection == DirRight))
			{
				dist = 0;
			}
		}
